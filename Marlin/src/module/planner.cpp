  ...
  void Planner::sync_fan_speeds(uint8_t (&fan_speed)[FAN_COUNT]) {
      ...
      #define _FAN_SET(F)                                                                                                                     \
        if (F == 0 && ENABLED(FAN0_IS_SERVO)) {                                                                                               \
          if (previous_fan0_speed != fan_speed[F]) {											                                                                    \
            previous_fan0_speed = fan_speed[F];											                                                                          \
            MOVE_SERVO(FAN0_SERVO_NO, (fan_speed[F] ? map(fan_speed[F], 1, 255, FAN0_SERVO_MIN, FAN0_SERVO_MAX) : FAN0_SERVO_MIN));           \
          }	                                                                                                                                  \
    ...
    void Planner::kickstart_fan(uint8_t (&fan_speed)[FAN_COUNT], const millis_t &ms, const uint8_t f) {
      ...
      if ((f > 0 || DISABLED(FAN0_IS_SERVO)) && fan_speed[f]) {
        if (fan_kick_end[f] == 0) {
